...
const flaskServerURL = 'https://scan-py.onrender.com'; // Replace with the actual URL of your Flask server


//send Email using Nodemailer

async function sendEmailWithPDF(resultValue, recipientEmail, dynamicHtml) {
  try {
    if (!recipientEmail) {
    ...
    axios.post(`${flaskServerURL}/result`)
      .then((response) => {
        ...
        if (response.data && response.data.result) {
          ...
          // Continue with your custom logic using the resultValue
          console.log("Your Result is: ", resultValue);
          ...
          sendEmailWithPDF(resultValue, recipientEmail, dynamicHtml)
           .then(() => {
               console.log('Email sent successfully!');
           })
             .catch((error) => {
        ...
        // req.session.name = req.body.name;
          // Render a page with the result
          res.render('result', { result: resultValue, email: req.session.email, name: req.session.name });
        } else {
          console.log('No valid result received from Flask server');
        }
      })
      .catch((error) => {
        console.error('Error making HTTP request to Flask server:', error);
      });
  } catch (err) {
    console.error(err);
